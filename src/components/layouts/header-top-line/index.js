...
const HeaderTopLine = ({ className, buttonText }) => {
  ...
  const isModelProfilePage =
    arrayPath && arrayPath.length > 1 && arrayPath[1].toLowerCase() === "model";
  const isGlobalPage =
    arrayPath &&
    arrayPath.length > 1 &&
    arrayPath[1].toLowerCase() === "global";
  const isAvatarPage =
    arrayPath &&
    arrayPath.length > 1 &&
    arrayPath[1].toLowerCase() === "avatarlibraries";
  const isMintingPage =
    arrayPath &&
    arrayPath.length > 1 &&
    arrayPath[1].toLowerCase() === "minting";
  const isOpenElementalsPage =
    arrayPath &&
    arrayPath.length > 1 &&
    arrayPath[1].toLowerCase() === "openelementals";

  const isBlackHeader =
    !isGlobalPage && !isAvatarPage && !isMintingPage && !isOpenElementalsPage;
  ...
  return (
    ...
    >
      ...
      <div className={styles.rightBox}>
        ...
        >
          <Link href="https://runway.digitalax.xyz">
            <a
              className={cn(
                styles.link,
                isBlackHeader ? "text-white" : "text-black"
              )}
              target="_blank"
          ...
          <div className={styles.signBtn}>
            {user ? (
              <div className={styles.buttonWrapper}>
                ...
                >
                  ...
                  >
                    <img
                      ...
                      src={`./images/icons/${
                        isBlackHeader ? "arrow-bottom" : "arrow-bottom-black"
                      }.svg`}
                      alt="arrow-bottom"
                    />
                  </button>
                ...
                {isShowMenu && (
                  <div className={styles.menuWrapper}>
                    ...
                    {isModel && (
                      <button
                        onClick={() => handleViewModelPageClick()}
                        className={styles.menuButton}
                      >
                        View Model Page
                      </button>
                    ...
                    {isModel && (
                      <button
                        onClick={() => handleEditModelPageClick()}
                        className={styles.menuButton}
                      >
                        Edit Model Page
                      </button>
              ...
              <Button
                ...
                background={isBlackHeader ? "transparent" : "black"}
              >
                {buttonText}
              </Button>
            )}
          ...
          <a className={styles.collapseIcon} onClick={onIconHander}>
            <img
              src={`/images/hamburger${isBlackHeader ? "" : "-black"}.png`}
              alt=""
            />
          </a>
        </div>
